...
class LinearHashTable<K, V> extends HashTableBase<K, V>
{
    ...
    public void addElement(K key, V value)
    {
        ...
        HashItem<K, V> currSlot = super.getItems().elementAt(hash);

        //finding the next free space, iterate until spot is null or key already exists 
        for(int i = 0 ; i < super.getItems().size()-1; i++){
            if(currSlot.isEmpty() || currSlot.getKey() == key){
                currSlot.setValue(value);
                currSlot.setKey(key);
                currSlot.setIsEmpty(false);
                _number_of_elements++;
                break;
            } 

            hash = (hash + 1) % super.getItems().size();
            currSlot = super.getItems().elementAt(hash);
        }
    ...
    public void removeElement(K key)
    {
        ...
        //finding the element to remove, iterate until key is found or out of vector bounds
        for(int i = 0; i < super.getItems().size()-1; i++){
            if(currSlot.getKey() == key){
                currSlot.isTrueEmpty();
                currSlot.setIsEmpty(true);
                _number_of_elements--;
                break;
            } 
